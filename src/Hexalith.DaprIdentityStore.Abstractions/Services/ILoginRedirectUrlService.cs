...
public interface ILoginRedirectUrlService
{
    ...
    /// </summary>
    /// <param name="returnUrl">The return URL to redirect to after login. If null, a default URL is used.</param>
    /// <returns>The redirect URL as a <see cref="Uri"/>.</returns>
    Uri GetRedirectUrl(Uri? returnUrl) => new($"/Account/Login?returnUrl={returnUrl}");
}